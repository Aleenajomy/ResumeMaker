...
- DO NOT start any sentence with "I". Use third-person or noun-first openings.
  ...
  Good: "Strong backend skills in REST API design, PostgreSQL, and authentication workflows."
  Good: "Proven ability to build and test scalable services with clean, maintainable code."
  Good: "Currently expanding knowledge of Docker to support containerised deployment pipelines."
  Bad: "I am a developer..." / "I have experience..." / "I focus on..."
- Sentence 1: Who the candidate is + top 2 skills (noun-first, no "I").
- Sentence 2: Core technical strength tailored to the JD using keywords from the allowed skills list.
- Sentence 3: A specific capability relevant to the JD (authentication, API design, database work, etc.).
- Sentence 4: Learning mindset or soft skill. If adding a new skill, place it here naturally.
- Include at least 4 keywords from the JD that exist in the allowed skills list.
- Avoid filler words: results-driven, passionate, motivated, detail-oriented, dynamic.
- Do not fabricate experience.
Skills section rules:
- Skills reordering is handled by the backend. Do NOT return a skills field.
Return strict JSON only:
{"headline": "string", "summary": "string", "changes_made": ["string"]}
headline: concise title tailored to the job (e.g. "Backend Engineer | Python | Django") or empty string if no headline in original.
"""

NON_TECH_LATEX_SECTION_SYSTEM_PROMPT = """
...
- Write exactly 4 sentences. 60-90 words total.
- DO NOT start any sentence with "I". Use third-person or noun-first openings.
  Good: "Operations graduate with strong coordination and stakeholder communication skills."
  Good: "Experienced in managing structured workflows, cross-team collaboration, and process execution."
  Good: "Adaptable work style with a focus on ownership, accountability, and on-time delivery."
  Good: "Actively developing skills in data reporting and process improvement tools."
  Bad: "I am..." / "I have..." / "I bring..." / "I am comfortable..."
- Sentence 1: Who the candidate is + top 2 strengths (noun-first, no "I").
...
Return strict JSON only:
{"headline": "string", "summary": "string", "changes_made": ["string"]}
headline: concise title tailored to the job (e.g. "Operations Associate | Coordination | Problem Solving") or empty string if no headline in original.
"""
...
STOPWORDS = {
    ...
    'you', 'your', 'their', 'they', 'this', 'those', 'these', 'we', 'our', 'us',
    # Generic job-posting words
    'role', 'job', 'work', 'team', 'skills', 'experience', 'years', 'required', 'preferred',
    'about', 'above', 'across', 'after', 'against', 'all', 'also', 'among', 'any', 'awards',
    'based', 'between', 'both', 'brand', 'but', 'can', 'clients', 'competitive', 'create',
    'diversity', 'do', 'does', 'each', 'etc', 'every', 'following', 'get', 'good', 'great',
    'help', 'high', 'how', 'if', 'including', 'into', 'just', 'keep', 'key', 'large',
    'like', 'look', 'make', 'many', 'may', 'more', 'most', 'must', 'need', 'new', 'not',
    'now', 'number', 'one', 'only', 'other', 'out', 'over', 'own', 'part', 'per', 'plus',
    'provide', 'related', 'same', 'see', 'set', 'should', 'since', 'so', 'some', 'such',
    'than', 'them', 'then', 'there', 'through', 'time', 'two', 'under', 'up', 'use',
    'used', 'using', 'various', 'very', 'via', 'want', 'what', 'when', 'where', 'which',
    'while', 'who', 'wide', 'within', 'without', 'would', 'yet',
}
...
The "email_body" must start with "Dear Hiring Team," and contain only the email body."""
    ...
    def build_latex_skills_section_update(skills_content: str, job_description: str) -> str:
        ...
        if not item_blocks:
            # Skills section has no \item blocks (e.g. plain textbf category lines).
            # Cannot reorder — return original so section is still written back unchanged.
        ...
        if any(score > 0 for score, _, _ in scored_items):
            ...
            # No JD match — keep original order but still return content so section is written
...
{{
  ...
  "changes_made": ["string"]
        ...
        # Always use pure Python reordering for skills — never trust AI to output valid LaTeX structure.
        ...
        if not isinstance(raw_changes, list):
            raw_changes = []
        changes_made = [str(item).strip() for item in raw_changes if str(item).strip()]
        if section_updates['skills'] and section_updates['skills'].split() != skills_content.split():
...
{{
  ...
  "changes_made": ["string"]
    ...
    def _extract_compound_terms(text: str) -> List[str]:
        """Extract hyphenated terms and meaningful bigrams from text."""
        normalized = text.lower()
        # Hyphenated terms like server-side, full-stack, back-end, etc.
        hyphenated = re.findall(r'[a-z][a-z0-9]*(?:-[a-z][a-z0-9]+)+', normalized)
        # Normalize them (remove hyphens) so matching is flexible
        normalized_hyphenated = [re.sub(r'-', '', term) for term in hyphenated]
        # Common tech bigrams: "machine learning", "data science", "rest api", etc.
...
- Role-specific hard skills (e.g. backend development, API design, authentication)
- Relevant soft skills only if explicitly required (e.g. communication, problem-solving)

Rules:
- Prefer SHORT atomic keywords (1-2 words max): "scalable" not "scalable code", "front-end" not "front-end technologies"
- Do NOT include generic company culture words (diversity, belonging, growth, global)
- Do NOT include generic filler (competitive, environment, employees, enterprise)
- Do NOT include vague phrases (client success, creative innovative, fosters)
- Do NOT include company names, locations, or benefits
- Do NOT duplicate: if you have "ORM" and "object-relational mapping", pick one

Return JSON: {{"keywords": ["keyword1", "keyword2", ...]}}
Limit to 20 most important keywords.

Job Description:
{AIService._truncate_text(job_description, 4000)}"""

            result = AIService._call_openai_with_retry(prompt, temperature=0.1)
            raw_keywords = result.get('keywords', [])
            if not isinstance(raw_keywords, list) or not raw_keywords:
                raise ValueError('empty')
            ai_keywords = [str(k).strip().lower() for k in raw_keywords if str(k).strip()]
        except Exception:
            # Fallback to token-based if AI fails
            ai_keywords = []

        if not ai_keywords:
            # Fallback: frequency-based with strict filtering
            jd_tokens = AIService._tokenize_keywords(job_description)
            frequency: Dict[str, int] = {}
            for token in jd_tokens:
                frequency[token] = frequency.get(token, 0) + 1
            meaningful = {kw for kw, freq in frequency.items() if freq > 1 or len(kw) >= 6}
            ai_keywords = sorted(meaningful, key=lambda t: frequency[t], reverse=True)[:25]

        resume_token_set = set(AIService._tokenize_keywords(tailored_resume_text))
        resume_normalized = re.sub(r'[^a-z0-9 ]', ' ', tailored_resume_text.lower())
        resume_normalized = re.sub(r'\s+', ' ', resume_normalized)

        def _keyword_in_resume(kw: str) -> bool:
            # Normalize keyword: lowercase, collapse spaces
            norm_kw = re.sub(r'[^a-z0-9 ]', ' ', kw.lower())
            norm_kw = re.sub(r'\s+', ' ', norm_kw).strip()
            if not norm_kw:
                return False
            # Exact phrase match in normalized resume
            if norm_kw in resume_normalized:
                return True
            # Single token match
            if norm_kw in resume_token_set:
                return True
            parts = [p for p in norm_kw.split() if len(p) >= 4]
            if not parts:
                return False
            # All parts present (multi-word phrase)
            if all(p in resume_normalized for p in parts):
                return True
            # For 2-word phrases: if the primary/longest word matches, count it
            # e.g. "scalable code" -> "scalable" in resume = match
            # e.g. "front-end technologies" -> "front" in resume = match
            if len(parts) <= 2:
                primary = max(parts, key=len)
                if primary in resume_normalized:
                    return True
            # Singular/plural
            stripped = norm_kw.rstrip('s')
            if len(stripped) >= 4 and stripped in resume_normalized:
                return True
            return False

        matched = sorted([kw for kw in ai_keywords if _keyword_in_resume(kw)])
        missing = sorted([kw for kw in ai_keywords if not _keyword_in_resume(kw)])

        score = round((len(matched) / len(ai_keywords)) * 100) if ai_keywords else 0
        return {
