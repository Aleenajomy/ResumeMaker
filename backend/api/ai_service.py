...
- Analyze the Current Summary Section structure. If it uses LaTeX commands (e.g. \\resumeItemListStart, \\resumeItem, itemize), preserve that exact wrapper structure and only replace the text content inside.
- If the summary is plain prose (no \\resumeItem wrappers), write 4 complete prose sentences.
- Write exactly 4 sentences. Minimum 60 words, maximum 90 words.
- The summary must read naturally like a human-written LinkedIn summary or professional paragraph — NOT like bullet points joined as sentences.
- Do NOT start every sentence with an action verb. Use a natural mix of sentence structures.
- First sentence MUST be a strong professional introduction describing who the candidate is (e.g. "Backend-focused Software Engineer with experience in..."). Do not start the first sentence with a verb like Built/Developed/Designed.
- Only 1 or 2 sentences may start with action verbs; the rest must be descriptive or skill-focused.
- Avoid robotic patterns. Do not list technologies in every sentence. Combine related skills into natural flowing sentences.
- Avoid filler phrases like results-driven, passionate, motivated, detail-oriented, dynamic.
- ATS-optimized and tailored to the job description. Include at least 3 distinct technical keywords from the job description naturally within the sentences.
...
- If the job description mentions soft skills (e.g. communication, teamwork, problem-solving), include at most 1 naturally in the summary.
- Do not fabricate experience. Keep it truthful and aligned with a Computer Science graduate profile.
- Ensure no word or technology is repeated more than once across the entire summary.
- The summary should sound like it was written by a human professional, not generated by AI.
- Output must be production-quality. If any rule above is violated, regenerate internally before responding.
Skills section rules:
- The skills section is handled by a separate Python reordering step. Do NOT return a skills field.
- Leave skills entirely to the backend — do not include it in your output.
...
{{
  ...
  "changes_made": ["string"]
    ...
    def calculate_ats_score_from_text(job_description: str, tailored_resume_text: str) -> Dict[str, Any]:
        ...
        compound_terms = AIService._extract_compound_terms(job_description)

        frequency: Dict[str, int] = {}
        for token in jd_tokens:
            frequency[token] = frequency.get(token, 0) + 1
        for term in compound_terms:
            frequency[term] = frequency.get(term, 0) + 2
        ...
        def _keyword_in_resume(kw: str) -> bool:
            ...
            return bool(normalized_kw and normalized_kw in resume_normalized)

        matched = sorted([kw for kw in prioritized_keywords if _keyword_in_resume(kw)])
        missing = sorted([kw for kw in prioritized_keywords if not _keyword_in_resume(kw)])
